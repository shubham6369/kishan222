      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8 pb-20 border-b border-white/10">
            ...
            <Link href="/" className="flex items-center gap-4 group">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] uppercase tracking-[0.4em] text-white/40 font-bold">
                  Est. Rural Empowerment 2024
                </span>
              </div>
            </Link>
            
            <p className="text-white/60 font-body text-lg leading-relaxed max-w-md">
              The premier collective for the modern Indian farmer. We facilitate a global transition to regenerative agriculture while preserving our rich rural heritage.
            </p>
          ...
          <div className="lg:col-span-7 grid grid-cols-2 md:grid-cols-3 gap-12">
            {/* Exploration */}
            <div>
              <h4 className="font-serif text-xl font-bold mb-8 text-accent">Exploration</h4>
              <ul className="space-y-6 font-body text-white/50">
                {["Our Heritage", "Marketplace", "Membership", "Technical Support"].map((item, i) => (
                  <li key={i}>
            ...
            {/* Intelligence */}
            <div>
              <h4 className="font-serif text-xl font-bold mb-8 text-accent">Intelligence</h4>
              <ul className="space-y-6 font-body text-white/50">
                {["Soil Analytics", "Market Trends", "Govt Schemes", "Climate Tech"].map((item, i) => (
                  <li key={i}>
                    <Link href="#" className="hover:text-white transition-colors flex items-center gap-2 group">
                      {item} <ArrowUpRight className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </Link>
            ...
            <div className="col-span-2 md:col-span-1">
              <h4 className="font-serif text-xl font-bold mb-8 text-accent">Assistance</h4>
              <div className="space-y-8 font-body text-white/50">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <span className="text-xs">24/7 Digital Support</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="pt-12 flex flex-col md:flex-row justify-between items-center gap-8 text-xs font-bold tracking-[0.2em] uppercase text-white/30">
          <div className="flex gap-10">
            <Link href="/privacy" className="hover:text-accent transition-colors">Privacy Charter</Link>
            <Link href="/terms" className="hover:text-accent transition-colors">Membership Terms</Link>
            <Link href="/ethics" className="hover:text-accent transition-colors">Ethics & Transparency</Link>
          </div>
